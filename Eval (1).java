...
public class Eval {
	...
	for(DieFace die1 : DieFace.values()) {
		if(die1 == DieFace.invalid) { // skip if invalid because the main should only loop 27 times 
			continue; 
		}
		for(DieFace die2 : DieFace.values()) {
			if(die2 == DieFace.invalid) {// skip if invalid because the main should only loop 27 times
				continue; 
			}
			for(DieFace die3 : DieFace.values()) {
				if(die3 == DieFace.invalid) { // skip if invalid because the main should only loop 27 times
					continue;
				}
				double prob = s.rollProb(die1, die2, die3); // get roll probability 
				s.roll(die1, die2, die3); 
				double roll_val = Eval.value_rolled_hand(s, depth);
				val = val + prob * roll_val;
			}
		}
	}
	...
	if (s.current_player == Turn.computer && s.brains_collected == 2) { // we need to check the brains collected to see if it is worth proceeding
		return State.win_payoff;
	} else if (s.current_player == Turn.computer && s.blasts_collected == 2) { // also take into consideration the opponents hand
		return -State.win_payoff; 
	} else {
		return (value);
	}
